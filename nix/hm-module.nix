...
let
  ...
  # Render the defaults block
  # Matches the output of Config::to_hcl() in src/config/loader.rs
  defaultsBlock =
    "defaults {\n"
    + optionalString (cfg.settings.editor != null) "  editor = \"${cfg.settings.editor}\"\n"
    + optionalString (
      cfg.settings.secretsPath != null
    ) "  secrets_path = \"${cfg.settings.secretsPath}\"\n"
  ...
  renderProvider =
    ...
    let
      attrs =
        ...
        + optionalString (p.tokenEnv != null) "  token_env = \"${p.tokenEnv}\"\n";
    in
    "provider \"${id}\" {\n${attrs}}";

  # Render the full HCL config file
  toHcl =
    let
      providerBlocks = concatStringsSep "\n\n" (mapAttrsToList renderProvider cfg.providers);
    in
    "# jaws configuration file\n# Generated by Home Manager\n\n"
    + optionalString hasSettings "${defaultsBlock}\n"
...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    xdg.configFile."jaws/jaws.hcl" = mkIf hasConfig {
      text = toHcl;
    };
  };
}
